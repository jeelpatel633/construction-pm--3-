...
export default function HomePage() {
  ...
  const [tip, setTip] = useState(null); // { id, type: 'current'|'expected' }
  ...
  return (
    <div style={{padding:'60px 28px 28px', display:'flex', flexDirection:'column', gap:24}}>
      ...
      {/* ✅ Real Estate Project Table with Cash In / Out / Profit */}
      <div className="card">
        <div className="card-header">
          <div className="card-title">📊 Final Real Estate Project Table</div>
        </div>
        {projects.length === 0 ? (
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th className="right" style={{color:'var(--success)'}}>Cash In (Received)</th>
                  <th className="right" style={{color:'var(--danger)'}}>Cash Out (Expense)</th>
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td className="right" style={{color:f(p.profit_current)>=0?'#7C3AED':'var(--danger)', fontFamily:'var(--font-mono)', fontWeight:700}}>
                        <span onClick={()=>setTip(tip?.id===p.id&&tip?.type==='current'?null:{id:p.id,type:'current'})}
                          style={{cursor:'pointer', borderBottom:'1px dashed currentColor'}}>
                          {INR(p.profit_current)}
                        </span>
                        {tip?.id===p.id&&tip?.type==='current' && (
                          <div style={{marginTop:6,background:'#F3E8FF',border:'1px solid #DDD6FE',borderRadius:8,padding:'8px 12px',fontSize:12,color:'#4C1D95',textAlign:'left',fontFamily:'var(--font)',fontWeight:400,whiteSpace:'nowrap'}}>
                      ...
                      <td className="right" style={{color:'#0EA5E9', fontFamily:'var(--font-mono)', fontWeight:600}}>
                        <span onClick={()=>setTip(tip?.id===p.id&&tip?.type==='expected'?null:{id:p.id,type:'expected'})}
                          style={{cursor:'pointer', borderBottom:'1px dashed currentColor'}}>
                          {INR(p.profit_expected)}
                        </span>
                        {tip?.id===p.id&&tip?.type==='expected' && (
                          <div style={{marginTop:6,background:'#EFF6FF',border:'1px solid #BFDBFE',borderRadius:8,padding:'8px 12px',fontSize:12,color:'#1E3A5F',textAlign:'left',fontFamily:'var(--font)',fontWeight:400,whiteSpace:'nowrap'}}>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
